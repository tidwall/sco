...
if [[ "$1" == "bench" ]]; then
    ...
    for f in *; do 
        ...
        $CC $CFLAGS -DSCO_NOAMALGA -o $f.test ../sco.c ../deps/llco.c $f
        if [[ "$WITHCOV" == "1" ]]; then
    ...
    if [[ "$WITHCOV" == "1" ]]; then
        ...
        $XCRUN llvm-cov report *.test ../sco.c -ignore-filename-regex=.test. \
            -j=4 \
            -show-functions=true \
        ...
        $XCRUN llvm-cov show *.test ../sco.c -ignore-filename-regex=.test. \
            -j=4 \
            -show-regions=true \
